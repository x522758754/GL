...
//视差贴图(Parallax Mapping)技术和法线贴图差不多
///但它有着不同的原则。和法线贴图一样视差贴图能够极大提升表面细节，使之具有深度感。
///它也是利用了视错觉，然而对深度有着更好的表达，与法线贴图一起用能够产生难以置信的效果。
///视差贴图和光照无关

///1.当前纹理代表最远采样点 texcoods
///2.相机的在切线空间观察方向 viewDir
///3.高度图代表纹理偏移的近似值 height
///由2和3，可以得纹理近似偏移向量为 P = -viewDir * height 
///最终纹理采样点：finalTexCoords = texcoods - P
...
class parallaxMapping : public app
{
	...
	void start()
	{
		shader = Shader("_shaders/_0405_parallaxMapping.vs", "_shaders/_0405_parallaxMapping.fs");
		...
		shaderNoraml.setInt("texture_height1", 2);
